...
class InstaClient:
    ...
    def Connect(self):
        ...
        if (self.IsCookiesOK() != True):
            PrintError("Çerezler alınamadı! Başka bir proxy deneyin! (Proxy:", self.user, self.ip, ":", self.port, ")")
        ...
        if (res.status_code == 200):
            try:
                ...
                if ("message" in obj and obj["message"] == "checkpoint_required"):
                    PrintError("Hesap doğrulaması gerektirir! (URL:", obj["checkpoint_url"], ")")
                    self.isproxyok = False
                    return
                ...
                if ("errors" in obj and "error" in obj["errors"]):
                    PrintError("Giriş başarısız oldu! Proxy çalışmıyor olabilir. (Proxy:", self.user, self.ip, ":", self.port, ")")
                    self.isproxyok = False
                    return
                self.isproxyok = False
            except:
    ...
    def Spam(self,userid, username, reasonid):
        ...
        try:
            ...
            if ("description" in obj and "status" in obj):
                if (obj["status"] == "ok" and obj["description"] == "Raporlarınız, topluluğumuzun spam'den uzak kalmasına yardımcı olur."):
                    PrintSuccess("Şikayet başarıyla gönderildi!", self.user)
                    return
            PrintError("Şikayet gönderme talebimiz reddedildi!", self.user)
        except:
            PrintError("Şikayet gönderirken bir hata oluştu!", self.user)
